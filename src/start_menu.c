...
extern const u8 gText_MenuAchievements[];
...
extern const u8 gText_AchievementMenuDescription[];
...
const struct MenuAction sStartMenuActionTable[] =
{
	...
	[STARTMENU_ALBUM] = {gText_MenuAchievements, {.u8_void = StartMenuAchievementMenuCallback}},
...
const u8* const sStartMenuDescPointers[] =
{
	...
	gText_AchievementMenuDescription,
...
static void BuildPokeToolsMenu(void)
{
	...
	if (FlagGet(FLAG_ACHIEVEMENT_MENU_GET))
		AppendToStartMenuItems(STARTMENU_ALBUM);
...
static bool8 CanSetUpSecondaryStartMenu(void)
{
	if (FlagGet(FLAG_SYS_DEXNAV))
		return TRUE;
	else if (FlagGet(FLAG_ACHIEVEMENT_MENU_GET))
		return TRUE;
	else
		return FALSE;
}
